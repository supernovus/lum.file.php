...
class File
{
  ...
  const CHMOD_TEST   = 'u+r';  // Minimal permissions allowed in chmod().
  const DIRMODE_TEST = 'u+rx'; // Minimal permissions allowed in setDirMode().
   ...
   *
   * If setting this directly (not recommended), it must be an integer
   * value representing the Unix permission value.
   *
   * The default is octal value 0755;
   *
   * If you want to set it using a string like 'u+rw,g+r' then see the
   * setDirMode() method.
   */
  public $dirMode = 0755;
  ...
  {
    // Make sure the mode is in a valid format.
    $mode = Permissions::parse($mode);

    // Ensure the specified mode is not too restrictive.
    if (Permissions::has($mode, self::DIRMODE_TEST))
    { 
      $this->dirMode = $mode;
    }
    else
    {
      throw new \Exception("setDirMode() missing required permissions: "
        . self::DIRMODE_TEST);
    }
  }
...
#      error_log("copied");
   ...
   *
   * @param int|string $mode  The permissions to set on the file.
   *                          May be a Unix permissions integer,
   *                          a 10 character permission string like 'ls' uses,
   *                          or a UGAO string like 'u=rwx,g+r,o-rx'.
   *
   *                          If it's a UGAO string, the operations are
   *                          applied to the current permissions, so if you
   *                          had a file with '-rw-r--r--' permissions, and
   *                          then call chmod('+x') you will end up with
   *                          permissions of '-rwxr-xr-x' when it's done.
   *
   * @return int|null  The new mode if it was applied, or null otherwise.
   */
  public function chmod ($mode)
  {
    if (isset($this->file) && file_exists($this->file))
    {
      $orig = fileperms($this->file);
      if (!is_int($orig)) $orig = 0;
      $mode = Permissions::parse($mode, ['inMode'=>$orig]);
      if (Permissions::has($mode, self::CHMOD_TEST))
      {
   ...
   */
  public function getStream ($mode=null, $flags=null)
  {
    return new Stream($this, $mode, $flags);
  }
