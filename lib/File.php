...
#      error_log("copied");
   ...
   *
   * @param int $mode=0  Mode to pass to open().
   *
   * @return mixed  If strict is true, will always return a ZipArchive on
   *                a successful operation. If strict is false, it may also
   *                return the ZipArchive error code.
   *
   * @throws Exception  If strict is true and a failure occurs, an Exception
   *                    will be thrown, see Lum\File\Zip for a list of
   *                    possible Exceptions that might be thrown.
   */
  public function getZip ($strict=true, $mode=0)
  {
    if ($strict)
    { // Strict mode uses the Lum\File\Zip wrapper.
      return Zip::open($this->file, $mode);
    }
    else
    { // Non-strict mode, simply uses ZipArchive directly.
      $zip = new \ZipArchive;
   ...
   *
   * @param string $prefix='zipfile'  Temp name prefix.
   * @param bool $stric=true  Should we use strict mode?
   * @param int $mode=0  Mode to pass to open().
   *
   ...
   */
  public function getZipDir ($prefix='zipfile_', $strict=true, $mode=0)
  {
    $zipfile  = $this->getZip($strict, $mode);
